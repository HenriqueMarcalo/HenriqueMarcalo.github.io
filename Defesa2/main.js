...
function carregarFiltros(produtos) {
  fetch('https://deisishop.pythonanywhere.com/categories/')
      ...
      .then(categorias => {
          ...
          ordemPadrao.textContent = 'Ordenar pelo rating';
      ...
      const descriptionTitulo = document.createElement('h3');
      descriptionTitulo.textContent = 'Descrição';
      filtrosContainer.append(descriptionTitulo);

      const tocarBtnDescription = document.createElement('button');
      tocarBtnDescription.textContent = 'Ocultar Descrição';
      tocarBtnDescription.id = 'tocar-botao-descricao';
      filtrosContainer.append(tocarBtnDescription);

      let descriptionVisivel = true;
      tocarBtnDescription.addEventListener('click', () => {
        const descricoes = document.querySelectorAll('.descricao');
        descricoes.forEach(descricao => {
            if(descriptionVisivel){
                tocarBtnDescription.textContent = 'Mostrar Descrições';
            } else {
                tocarBtnDescription.textContent = 'Ocultar Descrições';
            }
        });
        descriptionVisivel = !descriptionVisivel;
      });

      const addAllProductsBtn = document.createElement('button');
      addAllProductsBtn.textContent = 'Adicionar todos ao cesto';
      addAllProductsBtn.id = 'add-all';
      filtrosContainer.append(addAllProductsBtn);
